...
val cardDataStr = Source.fromFile(cardDataFile).mkString
import play.api.libs.json._
val json: JsValue = Json.parse(cardDataStr)
val card0 = (json \ "card")(6107).as[JsArray]

val cardData = parseJSToCardData(card0.value.toList)
...
def parseJSToCardData(data: List[JsValue]): CardDataJson = {
  ...
  val unk24 = it.next().as[JsNumber].value.toDouble // u5 24 // ??? Mostly 2.5
